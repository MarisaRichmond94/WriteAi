...
TEMPORAL_INSTRUCTION = """IMPORTANT: This is a point-in-time knowledge question. Only consider what has been revealed within {bound}. Distinguish carefully between what the READER knows and what the named CHARACTER has personally witnessed, been told, or deduced — answer for the character's knowledge, not the reader's. Answer only the specific aspect the question asks about: pick the notes and excerpts that bear on that topic and leave out the character's unrelated knowledge, even when the notes list plenty of it."""
